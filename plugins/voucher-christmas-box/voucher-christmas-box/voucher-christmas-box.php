...
add_action('init', function () {
    ...
    if ($recipient_name) {
        ...
        $pdf->SetTextColor(50, 50, 50);
        $pdf->SetXY(35, 155);
        $pdf->Cell(170, 10, $recipient_name, 0, 0, 'L');
    }
    ...
    if ($sender_name) {
        ...
        $pdf->SetXY(35, 130);
        $pdf->Cell(170, 10, $sender_name, 0, 0, 'L');
    }
    ...
    // ✅ Add Message (multi-line)
    if ($message) {
        $message_word  =  strlen($message);
        if($message_word <= 51){
            $pdf->SetXY(30, 220);
            $pdf->SetFont('Arial', '', 14);
            $pdf->MultiCell(115, 8, $message, 0, 'L');    
        }elseif($message_word <= 102){
            $pdf->SetFont('Arial', '', 14);
            $pdf->SetXY(30, 215);
            $pdf->MultiCell(115, 8, $message, 0, 'L');
        }elseif($message_word <= 153){
            $pdf->SetFont('Arial', '', 14);
            $pdf->SetXY(30, 210);
            $pdf->MultiCell(115, 8, $message, 0, 'L');
        }elseif($message_word <= 204){
            $pdf->SetFont('Arial', '', 13);
            $pdf->SetXY(30, 205);
            $pdf->MultiCell(115, 7, $message, 0, 'L');
        }else{
            $pdf->SetFont('Arial', '', 12);
            $pdf->SetXY(30, 205);
            $pdf->MultiCell(115, 6, $message, 0, 'L');
        }
